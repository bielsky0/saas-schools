...
import type { AuditAction, AuditTargetType } from "@/features/admin/audit";
...
/**
 * Rule A audit entry for a builder mutation. The ACTION/TARGET literals come
 * from `features/admin/audit.ts`'s unions on purpose: a call site that is not
 * in the registry is a compile error, so the audit surface cannot drift from
 * what the panel can render (spec 6.3).
 */
export type BuilderAuditEntry = {
  action: AuditAction;
...
/**
 * Everything the builder dispatcher needs besides the two thin HTTP layers.
 *
 * - `tx`/`organizationId` — supplied by `withTenant(orgId)`, so every tenant
 *   query is RLS-scoped to exactly that organization. `organizationId` is
 *   ALWAYS the tx's scope; the layers derive it differently (subdomain header
 *   vs. URL segment) but neither ever lets the client name it.
 * - `userId`/`role` — the acting identity. The tenant layer passes the
 *   session; the admin layer passes the super admin. `role` drives what the
 *   editor UI shows (`admin`/`guest`).
 * - `audit?` — OPT-IN Rule A auditor. The tenant editor (unchanged behaviour)
 *   passes nothing; the apex admin-editor passes a closure that records the
 *   write with a SuperAdmin actor in the SAME transaction. The core awaits it
 *   after every successful mutation, so a missing or failing audit aborts the
 *   write — never a silent unaudited change (spec 6.3).
 */
export type BuilderActionContext = {
  tx: TenantDb;
  organizationId: string;
  userId: string | null;
  role: "admin" | "guest";
  action: string;
  // The action payload is an untyped JSON object from `req.json()` (see the two
  // thin routes). The SDK's own dispatch contract is `{ action, data?: any }`;
  // every case below re-validates the fields it cares about.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  data?: any;
  audit?: (tx: TenantDb, entry: BuilderAuditEntry) => Promise<void>;
};

export async function handleBuilderAction(ctx: BuilderActionContext): Promise<Response> {
  const { tx, organizationId, userId } = ctx;
  ...
  switch (action) {
    ...
    case "CREATE_PAGE": {
      ...
      await ctx.audit?.(tx, {
        action: "page.create",
        targetType: "page",
        targetId: newPage[0]!.id,
        targetLabel: newPage[0]!.slug,
        metadata: { pageType: newPage[0]!.pageType, name },
      });
    ...
    case "CREATE_COLLECTION_ITEM": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetId: newPage[0]!.id,
        targetLabel: newPage[0]!.slug,
        metadata: { pageType: collection.pageType, collectionKey: collection.key, title },
      });
      return NextResponse.json({ page: toChaiPage(newPage[0]!) });
    }
    ...
    case "UPDATE_TEMPLATE": {
      ...
      await ctx.audit?.(tx, {
        action: "collection_template.update",
        targetType: "cms_collection",
        targetId: template.id,
        targetLabel: template.name,
        metadata: { collectionKey: collection.key, pageType: collection.templatePageType },
      });
    ...
    case "CREATE_COLLECTION": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetLabel: inserted[0]!.key,
        metadata: { name, pageType, templatePageType },
      });
    ...
    case "UPDATE_COLLECTION": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetLabel: collection.key,
        metadata: { name: set.name ?? collection.name, pageType: set.pageType ?? collection.pageType },
      });
    ...
    case "CREATE_COLLECTION_TEMPLATE": {
      ...
      const updated = await tx
        ...
        .returning();
      await ctx.audit?.(tx, {
        action: "collection_template.create",
        targetType: "cms_collection",
        targetId: collection.id,
        targetLabel: collection.key,
        metadata: { templateName: name, templateId: id },
      });
      return NextResponse.json({ collection: updated[0] });
    }
    ...
    case "UPDATE_COLLECTION_TEMPLATE": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetId: collection.id,
        targetLabel: collection.key,
        metadata: { templateId, templateName: existing.name },
      });
      return NextResponse.json({ collection: updated[0] });
    }
    ...
    case "DELETE_COLLECTION_TEMPLATE": {
      ...
      const updated = await tx
        ...
        .returning();
      await ctx.audit?.(tx, {
        action: "collection_template.delete",
        targetType: "cms_collection",
        targetId: collection.id,
        targetLabel: collection.key,
        metadata: { templateId },
    ...
    case "UPDATE_PAGE": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetLabel: updated[0]!.slug,
        metadata: { pageType: updated[0]!.pageType, titleChanged: updated[0]!.title !== existing.title },
      });
    ...
    case "UPDATE_PAGE_METADATA": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetLabel: newSlug,
        metadata: { pageType: existing.pageType, fields: ["title", "slug", "seo"] },
      });
      return NextResponse.json({ success: true });
    }
    ...
    case "DELETE_PAGE": {
      ...
      await ctx.audit?.(tx, {
        ...
        metadata: { pageType: existing.pageType, title: existing.title },
      });
    ...
    case "DUPLICATE_PAGE": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetType: "page",
        targetId: copy[0]!.id,
        targetLabel: copy[0]!.slug,
        metadata: { sourcePageId: original.id, sourceLabel: original.slug },
      });
    ...
    case "TAKE_OFFLINE": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetId: updated[0]!.id,
        targetLabel: updated[0]!.slug,
        metadata: { pageType: updated[0]!.pageType },
      });
      return NextResponse.json({
        tags: [],
    ...
    case "MARK_AS_TEMPLATE": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetId: updated[0]!.id,
        targetLabel: updated[0]!.slug,
        metadata: { pageType: "template", from: markExisting.pageType },
      });
      return NextResponse.json({ page: toChaiPage(updated[0]!) });
    }
    ...
    case "UNMARK_AS_TEMPLATE": {
      ...
      await ctx.audit?.(tx, {
        ...
        targetId: updated[0]!.id,
        targetLabel: updated[0]!.slug,
        metadata: { pageType: "page", from: updated[0]!.pageType },
      });
      return NextResponse.json({ success: true });
    }
    ...
    case "UPDATE_WEBSITE_FIELDS": {
      ...
      if (theme) {
        await ctx.audit?.(tx, {
          ...
          metadata: { fields: Object.keys(theme ?? {}) },
        });
      }
      return NextResponse.json({ success: true });
    }
