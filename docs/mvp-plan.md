...
> **Status: zrealizowane (2026-09-05).** Szczegóły: `docs/architecture/stripe-connect.md`.
...
| F4 — Dashboard UX | ✅ | 2026-09-05 | Schedule Builder (`schedule-builder.tsx`, siatka Pon–Ndz × godziny, commit do `groupTypeRecurrence`) + Kreator Group Type w 5 krokach (`group-type-wizard.tsx` + `wizard-steps/{step-basics,step-schedule,step-pricing,step-policies,step-publish}`), strony listy/EDYCji `dashboard/group-types`. |
| F5 — Stripe Connect E2E | ✅ | 2026-09-05 | E2E `scripts/test-connect.ts` (7 scenariuszy: onboarding, booking, pakiet, subskrypcja+odnowienie, zwrot pełny/częściowy, group_change, extra_fee); panel Connect + test-mode (`STRIPE_TEST_MODE`); **monitoring webhooków**: migracja 0082 (status/attemptCount/lastError/lastAttemptAt/payload) + `recordWebhookFailure` w route + oparte na cronie `webhooks.monitor-stuck` (replay przez `processConnectWebhookEvent`, dead-letter po 3 próbach + alert `webhook-dead-lettered` do ownerów, seed 0083) + `WebhookHealthPanel` na stronie billing; naprawa hand-written migracji 0081/0082 (breakpoint w środku instrukcji). Szczegóły: `docs/architecture/stripe-connect.md`. |
