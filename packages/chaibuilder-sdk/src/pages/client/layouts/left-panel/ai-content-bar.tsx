...
/**
 * Natural-language content editor stub. When `askAiCallBack` is provided it
 * rewrites content from a prompt; until then it shows the "under construction"
 * toast (backend AI is wired in a later phase).
 */
export const AiContentBar = () => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await askAiCallBack("content", prompt, block ? [block] : [], fallbackLang);
      if (response?.error) {
        toast.error(t("Something went wrong"));
        return;
      }
      // TODO(backend AI): apply response.blocks (content props) to the block store.
      toast.success(t("Content applied"));
      setPrompt("");
    } catch {
      toast.error(t("Something went wrong"));
    } finally {
  ...
  return (
    <div className="mb-3 rounded-lg border border-blue-100 bg-blue-50/60 p-2">
      <div className="mb-1.5 flex items-center gap-1 px-0.5 text-[11px] font-medium text-blue-700">
        ...
        <span>{t("Rewrite or improve content")}</span>
      </div>
      <div className="flex items-center gap-1.5">
        <input
          ...
          placeholder={t("e.g. make the headline friendlier and shorter")}
